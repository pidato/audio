...
var (
	...
	alaw2lpcmPointer = unsafe.Pointer(&alaw2ulaw[0])
)

func DecodeALAW(dst []int16, src []byte) []int16 {
	if len(src) == 0 {
		return nil
	}
	if len(dst) < len(src) {
	...
	for i := 0; i < len(dst); i++ {
		*(*int16)(unsafe.Add(d, i*2)) = *(*int16)(unsafe.Add(alaw2lpcmPointer, *(*byte)(unsafe.Add(s, i))))
	}
	return dst
}

// EncodeALAW encodes 16bit LPCM data to G711 A-law PCM
func EncodeALAW(dst []byte, src []int16) []byte {
	if len(src) == 0 {
		return nil
	}
	if len(dst) < len(src) {
	...
	for i := 0; i < len(dst); i++ {
		*(*byte)(unsafe.Add(d, i)) = encodeALAWFrame(*(*int16)(unsafe.Add(s, i*2)))
	}
	return dst
}
...
func (alaw) PayloadType() uint8 {
	return 8
}

func (alaw) Name() string {
	return "PCMA/8000"
}
